...
exports.handler = async (event) => {
    ...
    try {
        ...
        if (!customer || customer.trim().length < 3)
            return respond(400, { error: 'اسم العميل غير صحيح' });

        if (!phone || !/^[0-9]{10,15}$/.test(phone.trim()))
            return respond(400, { error: 'رقم الهاتف غير صحيح' });

        if (!address || address.trim().length < 3)
            return respond(400, { error: 'العنوان مطلوب' });

        if (!governorate)
            return respond(400, { error: 'المحافظة مطلوبة' });

        if (!['delivery', 'online', 'cash'].includes(paymentMethod))
            return respond(400, { error: 'طريقة دفع غير صحيحة' });

        if (!cartItems || !Array.isArray(cartItems) || cartItems.length === 0)
            return respond(400, { error: 'السلة فارغة' });
        ...
        for (const item of cartItems) {
            if (!item.productId) {
                return respond(400, { error: 'بيانات المنتج غير صحيحة' });
            }

            const productDoc = await db.collection('products').doc(item.productId).get();

            if (!productDoc.exists) {
                return respond(400, { error: `المنتج غير موجود: ${item.productId}` });
            }

            const product = productDoc.data();

            if (product.hidden === true) {
                return respond(400, { error: 'أحد المنتجات غير متاح حالياً' });
            }
        ...
        let shipping = 0;
        const shippingDoc = await db.collection('settings').doc('governoratesPricing').get();

        if (shippingDoc.exists) {
            const prices = shippingDoc.data().prices || {};
            const govKey = Object.keys(prices).find(k => k.trim() === governorate.trim());
            if (govKey !== undefined) {
                shipping = parseFloat(prices[govKey]) || 0;
            }
        }
        ...
        // ── 9. Return success with server-calculated totals ──────
        return respond(200, {
            success: true,
            orderId: orderRef.id,
            total: finalTotal,
        ...
        return respond(500, { error: 'حدث خطأ في السيرفر. حاول مرة أخرى.' });
    }
};
