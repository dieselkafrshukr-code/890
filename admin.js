document.addEventListener('DOMContentLoaded', () => {
    ...
    const PERMISSIONS = {
        ...
        'store@eltoufan.com': ['products', 'categories', 'governorates', 'coupons', 'site-alerts'] // Content Manager
    };
    ...
    function loadTab(tab) {
        ...
        if (tab === 'site-alerts') renderSiteAlerts();
    ...
    // --- 5.5. SPECIAL OFFERS ---
    // --- 5.5. WEB ALERTS (Replaces Special Offers) ---
    async function renderSiteAlerts() {
        tabContent.innerHTML = `
            <div class="actions-header">
                <h3>📢 شريط التنبيهات العلوي (Announcement Bar)</h3>
                <div id="ann-status-badge" style="padding:10px 20px; border-radius:12px; font-weight:700;">جاري التحميل...</div>
            </div>
            
            <div class="settings-card" style="background:var(--card); padding:24px; border-radius:20px; border:1px solid var(--border); margin-top:20px;">
                <div class="input-group" style="margin-bottom:20px;">
                    <label>نص التنبيه (مثال: خصم 25% على جميع المنتجات):</label>
                    <textarea id="ann-text-input" rows="3" placeholder="اكتب الرسالة التي ستظهر في أعلى الموقع هنا..."></textarea>
                </div>
                
                <div style="display:flex; gap:15px; flex-wrap:wrap;">
                    <button id="save-ann-btn" class="add-btn" style="flex:1; height:50px;">
                        <i data-lucide="save"></i> حفظ النص وتحديث الموقع
                    </button>
                    <button id="toggle-ann-btn-main" class="save-btn" style="flex:1; height:50px; background:#4caf50;">
                        <i data-lucide="power"></i> تفعيل / إيقاف التنبيه
                    </button>
                </div>
                <p style="color:var(--text-dim); font-size:0.85rem; margin-top:15px; line-height:1.6;">💡 هذا الشريط سيظهر في أعلى الموقع للزوار على جميع الصفحات. زر (X) في الموقع يسمح للزائر بإخفائه مؤقتاً.</p>
            </div>
        `;
        ...
        function updateAlertUI(enabled) {
            ...
            const btn = document.getElementById('toggle-ann-btn-main');
            isEnabled = enabled;
            if (enabled) {
                badge.innerHTML = '🟢 التنبيه يعمل الآن';
                badge.style.background = 'rgba(76,175,80,0.1)';
                badge.style.color = '#4caf50';
                btn.innerHTML = '<i data-lucide="power"></i> إيقاف التنبيه الآن';
                btn.style.background = '#f44336';
            } else {
                badge.innerHTML = '⚪ التنبيه متوقف حالياً';
                badge.style.background = 'rgba(128,128,128,0.1)';
                badge.style.color = 'var(--text-dim)';
                btn.innerHTML = '<i data-lucide="power"></i> تفعيل التنبيه الآن';
                btn.style.background = '#4caf50';
            }
            lucide.createIcons();
        }

        document.getElementById('save-ann-btn').onclick = async () => {
    ...
    async function renderSettings() {
        tabContent.innerHTML = `
            ...
            <div class="settings-grid" style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:20px; margin-top:2rem;">
                ...
                <div class="settings-card" style="background:var(--card); padding:24px; border-radius:20px; border:1px solid var(--border);">
                    <div style="font-weight:900; font-size:1.1rem; margin-bottom:20px; color:var(--accent); display:flex; align-items:center; gap:10px;">
                        <i data-lucide="info"></i> ملاحظة حول التنبيهات
                    </div>
                    <p style="color:var(--text-dim); font-size:0.8rem; margin-top:10px; line-height:1.4;">💡 لإدارة شريط الإعلانات العلوي، يرجى الانتقال إلى تبويب <b>"تنبيهات الموقع"</b> من القائمة الجانبية.</p>
                </div>
        ...
        // Announcement logic handled in dedicated tab
